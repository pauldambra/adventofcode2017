...
val puzzle_input = "179\t2358\t5197\t867\t163\t4418\t3135\t5049\t187\t166\t4682\t5080\t5541\t172\t4294\t1397\n" +
...
object Day2Tests : Spek({
    describe("day2") {
        on("part one") {
            it("solves the example input") {
                CheckSum().calculate("5\t1\t9\t5\n" +
                        "7\t5\t3\n" +
                        "2\t4\t6\t8").should.equal(18)
            }
            it("prints out the puzzle output") {
                val message = CheckSum().calculate(puzzle_input)
                println("day two part one result is $message")
            }
        }
        on("part two") {
            it("solves the example input") {
                CheckSum().calculatePartTwo("5\t9\t2\t8\n" +
                        "9\t4\t7\t3\n" +
                        "3\t8\t6\t5").should.equal(9)
            }
            it("prints out the puzzle output") {
                val message = CheckSum().calculatePartTwo(puzzle_input)
                println("day two part two result is $message")
            }
        }
        on("finding evenly divisible numbers") {
            it("can do 8 and 2") {
                val numbers = listOf(8,2)
                findFirstEvenlyDivisible(numbers).should.equal(4)
            }
            it("can do 8 and 4 and 2") {
                val numbers = listOf(8,4,2)
                findFirstEvenlyDivisible(numbers).should.equal(2)
            }
        }
    }
})
