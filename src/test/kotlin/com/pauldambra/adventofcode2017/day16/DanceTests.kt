...
data class Group(var state: MutableList<Char>) {
    ...
    val seenSteps = mutableMapOf<List<Char>, List<Char>>()

    fun singleDanceStep(danceMove: String) {

        val key = state.toList()+danceMove.toCharArray().toList()
        if (seenSteps.containsKey(key)) {
            state = seenSteps[key]!!.toMutableList()
        } else {
            ...
            seenSteps.put(key, state.toList())
        }
    }
